...
#include "RenderablePrimitives/ImmediateTriangle.h" //replacing ImmediateTriangleRenderer.h, not sure if corect
// #include "../InteractableDemo.h" //included interactable demo base in header...
#include "RenderablePrimitives/ClickableVisualVector.h"
#include "Framework/DrawVectorDemo.h"
#include "RenderablePrimitives/ClickableVisualPoint.h"
#include "RenderablePrimitives/ClickableVisualRay.h"
#include "RenderablePrimitives/VectorProjectionAnimation.h"
#include "RenderablePrimitives/VectorVisualizationTools/VectorGridLines.h"
#include "RenderablePrimitives/InteractiveMatrix.h"
////////////////////////////////////////////
...
namespace ray_tri_ns
{
	...
	/** Note, this was a "WindowManager", but chnaged base class.  */
	class RayTriDemo final : public TutorialEngine::InteractableDemoBase
	{
	public:
		static sp<Engine::Montserrat_BMF> font;
		static sp<QuaternionCamera> quatCam;
		static up<TutorialEngine::LineRenderer> lineRenderer;
	public:
		...
		virtual void gatherInteractableCubeObjects(
			std::vector<const TriangleList_SNO *> &objectList) override;

	private:
		up<Engine::Shader> coneTipShader = nullptr;
        ...
        up<TutorialEngine::LineRenderer> RayTriDemo::lineRenderer = nullptr;
	sp<Engine::Montserrat_BMF> RayTriDemo::font = nullptr;
	sp<QuaternionCamera> RayTriDemo::quatCam = nullptr;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	protected:
		sp<TutorialEngine::ImmediateTriangle> genericTriangle = nullptr;
		sp<TutorialEngine::VisualVector> genericVector = nullptr;
		sp<TutorialEngine::VisualPoint> genericPoint = nullptr;
		sp<Engine::TextBlockSceneNode> genericText = nullptr;
	};
	...
	void Slide_AllRenderables::init()
	{
		...
		genericTriangle = new_sp<TutorialEngine::ImmediateTriangle>();
		genericVector = new_sp<TutorialEngine::VisualVector>();
		genericPoint = new_sp<TutorialEngine::VisualPoint>();
		genericText = new_sp<Engine::TextBlockSceneNode>(RayTriDemo::font, "0.f");
	}
	...
	{
		// using TutorialEngine::SceneNode_TriangleList;
		...
		virtual void init() override
		{
			...
			grandparent = new_sp<SceneNode_TriangleList>(new_sp<TutorialEngine::TriangleList_SNO>(TutorialEngine::TriangleCube{}.triangles));
			parent = new_sp<SceneNode_TriangleList>(new_sp<TutorialEngine::TriangleList_SNO>(TutorialEngine::TriangleCube{}.triangles));
			child = new_sp<SceneNode_TriangleList>(new_sp<TutorialEngine::TriangleList_SNO>(TutorialEngine::TriangleCube{}.triangles));
...
// 	}
	...
	{
		InteractableDemoBase::init(); //added to register other virtuals...
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// had to add this, as camera was initialized elsewhere
		sp<Engine::ICamera> camera = Engine::RenderSystem::get().getRenderCamera(0);
		// if (QuaternionCamera* qCam = dynamic_cast<QuaternionCamera*>(camera))
		if (sp<QuaternionCamera> qCam = std::dynamic_pointer_cast<QuaternionCamera>(camera))
		{
			quatCam = qCam;
			qCam->pos = glm::vec3(0,0,5); //todo: should this just be an interface function?
		}
		// disable left click select, I must have changed this behavior for some other demo
		bLeftMouseCanSelect = false;
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////

		// frameRenderData.window = window; disabling, as frame render data is handled differently.
	...
	void RayTriDemo::gatherInteractableCubeObjects(
            std::vector<const TriangleList_SNO *> &objectList) 
	{
		//note: added this just because it was abstract in parent class and causing issues. it was not part of the original port.
	}
	...
	void RayTriDemo::render_game(float dt_sec)
	{
		...
		// -- disabling this, as I think the render system handles it now--
		// rd->view = quatCam->getView();
		// rd->projection = glm::perspective(quatCam->getFOVy_rad(), rd->aspect, near, far);
		// rd->projection_view = rd->projection * rd->view;
		// rd->camera = quatCam.get();
		...
		{
			vector.render(rd->projection_view, camPos);
		}
	...
	{
		// Disabled this as the engine system handles settuing up a new frame, and we get a crash saying forgot to call end frame
		...
		/////////////////////////////////////////////////////////////////////////////////////
		{
			...
			if (bFirstDraw)
			{
				bFirstDraw = false;
				const sp<FrameRenderData>& rd = RenderSystem::get().getFrameRenderData();
				ImGui::SetNextWindowPos(ImVec2{ 0, (rd->fbHeight * 0.9f) });
			}
		...
		// disabling, as the engine handles this.
	...
	void Slide_HighlevelOverview::init()
	{
		...
		ray = new_sp<TutorialEngine::ClickableVisualRay>();
		...
		pointRenderer = new_sp<TutorialEngine::VisualPoint>();
		planeRenderer = new_sp<TutorialEngine::PlaneRenderer>();

		textRenderer = new_sp<Engine::TextBlockSceneNode>(RayTriDemo::font, "text.");
	...
	void Slide_HighlevelOverview::render_game(float dt_sec)
	{
		...
		{
			// TODO: gles2 does not have glPolygonMode!
			bool oldWireFrameValue = triRender->bWireFrame;
			triRender->bWireFrame = bTriWireFrame;
			...
			triRender->bWireFrame = oldWireFrameValue;
		}
	...
	void Slide_HighlevelOverview::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
	...
	void Slide_RayReview::render_game(float dt_sec)
	{
		...
		if (camera)
		{
			ray->render(rd->projection_view, camera->getPosition());
		}
	}
	...
	void Slide_RayReview::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
		SlideBase::gatherInteractableCubeObjects(objectList);
	...
	void Slide_DotProductReview::init()
	{
		...
		dotProductValue = new_sp<Engine::TextBlockSceneNode>(RayTriDemo::font, "0.f");
	}
	...
	void Slide_CrossProductReview::init()
	{
		...
		crossProductText = new_sp<Engine::TextBlockSceneNode>(RayTriDemo::font, "0.f");
	...
	void Slide_CrossProductReview::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
		SlideBase::gatherInteractableCubeObjects(objectList);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void Slide_PlaneEquation::init()
	{
		...
		planeNormal = new_sp<TutorialEngine::ClickableVisualVector>();
		...
		vecToPoint = new_sp<TutorialEngine::ClickableVisualVector>();
		...
		text_dotProductValue = new_sp<Engine::TextBlockSceneNode>(RayTriDemo::font, "0.f");
	}

	void Slide_PlaneEquation::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
	...
	// live coding
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void Slide_LiveCoding::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
		SlideBase::gatherInteractableCubeObjects(objectList);
	...
	void Slide_LiveCoding::render_game(float dt_sec)
	{
		...
		if (camera)
		{
			...
			genericVector->render(rd->projection_view, camera->getPosition());
		}
	}
	...
	bool Slide_LiveCoding::liveCodingIntersection()
	{
		...
		vec3 cameraDirection = camera->getFront();
	...
	void Slide_VectorProjectionExplanation::init()
	{
		...
		aVec = new_sp<ClickableVisualVector>();
		bVec = new_sp<ClickableVisualVector>();
		projectionAnim = new_sp<TutorialEngine::VectorProjectionAnimation>();
		...
		text = new_sp<Engine::TextBlockSceneNode>(RayTriDemo::font, "0.f");
	...
	void Slide_VectorProjectionExplanation::render_game(float dt_sec)
	{
		...
		if (camera)
		{
			aVec->render(rd->projection_view, camera->getPosition());
			bVec->render(rd->projection_view, camera->getPosition());

			if (QuaternionCamera* quat_camera = dynamic_cast<QuaternionCamera*>(camera))
			{
				text->setLocalRotation(quat_camera->getRotation());
				text->setLocalScale(vec3(5.f));
				text->wrappedText->text = "vector projection review";
				//text->render(rd->projection, rd->view);
			}

			projectionAnim->render(rd->projection_view, camera->getPosition());
		}
	}
	...
	void Slide_VectorProjectionExplanation::render_UI(/*float dt_sec*/)
	{
		...
		ImGui::Begin("Vector projections", nullptr, flags);
		{
			...
			{
				if (const sp<TutorialEngine::ClickableVisualVector>& selection = getSelection())
				{
			...
			if (projectionAnim->animMode == TutorialEngine::EAnimMode::FASTER_TIP)
			{
				ImGui::SameLine();
			...
			{
				projectionAnim->animMode = static_cast<TutorialEngine::EAnimMode>(proxyAnimMode);
			}
	...
	void Slide_VectorProjectionExplanation::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
	...
	void Slide_ProjectionOnAxesGrid::init()
	{
		...
		projectionAnim_x = new_sp<TutorialEngine::VectorProjectionAnimation>();
		projectionAnim_x->setColor(vec3(1, 0, 0));

		projectionAnim_y = new_sp<TutorialEngine::VectorProjectionAnimation>();
		projectionAnim_y->setColor(vec3(0, 1, 0));

		projectionAnim_z = new_sp<TutorialEngine::VectorProjectionAnimation>();
	...
	void Slide_ProjectionOnAxesGrid::tick(float dt_sec)
	{
		...
		if (camera)
		{
			glm::vec3 fromOriginToCamera = camera->getPosition();//consider this a vector from 0;
	...
	void Slide_ProjectionOnAxesGrid::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
		Parent::gatherInteractableCubeObjects(objectList);
	...
	void Slide_BaryCentricsExplanation::render_game(float dt_sec)
	{
		...
		bool oldWireFrame = triRender->bWireFrame;
		...
		triRender->bWireFrame = oldWireFrame;
	...
	void Slide_BaryCentricsExplanation::renderGame_Barycentric_myMethod(float dt_sec)
	{
			...
			, const sp<TutorialEngine::ClickableVisualPoint>& c)
		{
			...
			{
				glm::vec3 camPos = camera->getPosition();
				glm::vec3 camOffset = camera->getUp()*0.01f; //create slight offset so that there isn't zfighting on lines
...
#define COMPILETIME_SQUARE(val) val * val
	...
	void Slide_BaryCentricsExplanation::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
		SlideBase::gatherInteractableCubeObjects(objectList);
	...
	void Slide_BaryCentricsExplanation::handleTestPointUpdated(const TutorialEngine::VisualPoint& pnt)
	{
	...
	void Slide_BaryCentricsExplanation::helper_renderVector(bool bShouldRender, glm::vec3 start, glm::vec3 dir, glm::vec3 color)
	{
		...
		if(bShouldRender)
		{
			...
			Engine::ICamera* camera = Engine::RenderSystem::get().getRenderCamera(0).get();
			genericVector->render(rd->projection_view, camera ? camera->getPosition() : vec3(0.f));
		}
	}
	...
	void Slide_BaryCentricsExplanation::helper_renderVector(bool bShouldRender, glm::vec3 start, glm::vec3 dir, glm::vec3 color, float timestampSecs, bool bDriftToOrigin)
	{
		...
		if (bShouldRender)
		{
			...
			genericVector->color = color;
			Engine::ICamera* camera = Engine::RenderSystem::get().getRenderCamera(0).get();
			genericVector->render(rd->projection_view, camera ? camera->getPosition() : glm::vec3(0.f));
		}
	}

	void Slide_BaryCentricsExplanation::helper_renderProjection(bool bShouldRender, TutorialEngine::VectorProjectionAnimation& projAnim, glm::vec3 aVec, glm::vec3 bVec, glm::vec3 aStart, glm::vec3 bStart, float dt_sec, std::optional<glm::vec3> color)
	{
		using namespace glm;
		const sp<FrameRenderData>& rd = RenderSystem::get().getFrameRenderData();
		if (bShouldRender)
		{
			...
			Engine::ICamera* camera = Engine::RenderSystem::get().getRenderCamera(0).get();
			projAnim.render(rd->projection_view, camera ? camera->getPosition() : glm::vec3(0.f)); //prevent flickering as it hasn't been ticked yet
		}
		else
		{
	...
	void Slide_AllRenderables::render_game(float dt_sec)
	{
		...
		if (bRenderXYZ && rd)
		{
			...
			Engine::ICamera* camera = Engine::RenderSystem::get().getRenderCamera(0).get();
			std::optional<glm::vec3> camPos = camera ? camera->getPosition() : glm::vec3(0.f);
	...
	void Slide_VectorVsRay::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
	...
	void Slide_VectorVsRay::tick(float dt_sec)
	{
		...
		if (bLerpToOrigin)
		{
			for (sp<TutorialEngine::ClickableVisualVector>& vec : customVectors)
			{
				if (vec != newVector)
				{
	...
	void Slide_ScalarTripleProductReview::init()
	{
		...
		projAnim = new_sp<TutorialEngine::VectorProjectionAnimation>();
	...
	void Slide_ScalarTripleProductReview::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
		SlideBase::gatherInteractableCubeObjects(objectList);
	...
	void Slide_DeterminantReview::gatherInteractableCubeObjects(std::vector<const TutorialEngine::TriangleList_SNO*>& objectList)
	{
		SlideBase::gatherInteractableCubeObjects(objectList);
	...
	void Slide_DeterminantReview::init()
	{
		...
		matrix = new_sp<TutorialEngine::Matrix_3D>();
...
int main()
{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// The window configuration that was previously used
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// WindowManager::WindowParameters params;
	// params.windowResolution.x = 1400;
	// params.windowResolution.y = 900;
	RenderSystem::default_width = 1400;
	RenderSystem::default_height = 900;

	sp<TutorialUtils::TutorialEngine> engine = new_sp<TutorialUtils::TutorialEngine>();

	engine->start([&]() {
		static sp<Engine::SceneNode> applicationLifetimeObject = new_sp<ray_tri_ns::RayTriDemo>();
		// applicationLifetimeObject->init();
		engine->addToRoot(applicationLifetimeObject);

		if(ray_tri_ns::RayTriDemo* ray_tri_demo 
			= dynamic_cast<ray_tri_ns::RayTriDemo*>(applicationLifetimeObject.get()))
		{
			// modify EngineBase::makeInitialWindow?

			//I am unsure if this is needed, just mirroring the previous anim.start(); call
			// ray_tri_demo->start();
		}
	});
}